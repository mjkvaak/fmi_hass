...
python -m fmi_radar --theme dark --format both --cmap plasma
```
...
- `output/radar.gif` — T=−15…0 observed + T=+5/+10/+15 nowcast (`--gif`, or `python -m fmi_radar.nowcast`)
...
| `--cmap` | theme default | Any matplotlib colormap name (`plasma`, `turbo`, `YlGnBu`, …) |
...
| `--gif` | off | Write `output/radar.gif` (T=−15 … T=+15) |
...
Optical flow + animation (`output/radar.gif` by default; `--no-gif` to skip):

```bash
source .venv/bin/activate
python -m fmi_radar.nowcast --cmap turbo
python -m fmi_radar.nowcast --time 202609040300 --lat YOUR_LAT --lon YOUR_LON --cmap plasma
python -m fmi_radar --gif --theme dark --time 202609040300
```
...
2. Run every 5 minutes (systemd timer or cron) with `--format both --theme dark` into a directory Home Assistant can read (for example `/config/www/fmi_radar/` or a bind-mounted folder).
3. Publish MQTT (`FMI_RADAR_MQTT_HOST` + user/password in the environment, never in git).
4. Add MQTT sensors for `fmi_radar/status` and `fmi_radar/mean_rr`, and a `picture-entity` / `local_file` camera on `output.png` (PNG is more reliable in Lovelace than SVG).
